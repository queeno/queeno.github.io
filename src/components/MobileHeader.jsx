...
export function MobileHeader({ data }) {
  ...
  return (
    <div className="md:hidden bg-slate-50 border-b border-slate-200 p-8 text-center">
      ...
      <h1 className="text-3xl font-bold text-slate-900 mb-2">{personalInfo.name}</h1>
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-4 text-slate-600 text-sm font-medium">
        {personalInfo.roles.map((role, index) => (
          <span key={index} className="px-2 py-0.5 bg-slate-100 rounded-full border border-slate-200">
            {role}
          </span>
        ))}
      </div>
      ...
      <div className="flex justify-center gap-4">
        <a href={`https://github.com/${personalInfo.github}`} target="_blank" rel="noopener noreferrer" className="p-2 bg-white border border-slate-200 rounded-full text-slate-700 hover:text-blue-600 hover:border-blue-500 transition-all shadow-sm">
          <Github size={20} />
        </a>
