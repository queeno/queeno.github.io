...
export function Sidebar({ data }) {
  ...
  return (
    <motion.aside
      ...
      className="hidden md:flex flex-col items-center text-center bg-slate-50 text-slate-700 rounded-2xl shadow-xl shadow-slate-200 border border-slate-200 p-8 md:w-[320px] lg:w-[350px] shrink-0 md:sticky md:top-24 md:max-h-[calc(100vh-12rem)] overflow-y-auto custom-scrollbar"
    >
      ...
      >
        <div className="w-48 h-48 rounded-full overflow-hidden border-4 border-white shadow-xl mb-6 ring-1 ring-slate-200">
          <img
      ...
      <div className="space-y-2 mb-8">
        {personalInfo.roles.map((role, index) => (
          <p key={index} className="text-xs font-semibold bg-white text-slate-600 py-1.5 px-3 rounded-full inline-block m-1 border border-slate-200 shadow-sm">{role}</p>
        ))}
      </div>
      ...
      <div className="space-y-5 w-full flex flex-col items-center">
        ...
        <div className="flex gap-4 mt-6">
          <a href={`https://github.com/${personalInfo.github}`} target="_blank" rel="noopener noreferrer" className="p-3 bg-white border border-slate-200 rounded-full text-slate-600 hover:text-white hover:bg-slate-900 hover:border-slate-900 transition-all shadow-sm">
            <Github size={20} />
          </a>
